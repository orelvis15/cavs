...
Raw outputs of the v0.9.0 SteamPipe-style analysis benchmark suite
...
| xdelta3 | 3.2.0 |
| zstd (linked library) | 1.5.x via the `zstd` crate |
| butler | not installed for this run (routes report it as skipped) |
| Dataset seeds | steampipe-cases 9 · gen-dir 7 · gen/version-stream 5 |
| Date | 2026-07-07 |

`steampipe-cases` reports additionally embed this metadata per run
(OS, CPU, RAM, disk, tool versions, the exact command line and the
seed) in their own `## Environment` section — a result is never
anonymous.
...
**One command reproduces everything** — the [`scripts/`](scripts/)
directory contains one script per benchmark, all datasets generated
from fixed seeds (no `/dev/urandom` anywhere):

```sh
cargo build --release -p cavs-cli
CAVS=$PWD/target/release/cavs docs/results/v0.9.0/scripts/run-all.sh /tmp/v090-results
# optionally: BUTLER=/path/to/butler to include the butler routes
```

| Script | Benchmark |
|---|---|
| `bench-abcg-steampipe-cases.sh` | A, B, C, G — pathology cases (seed 9) |
| `bench-d-depot-sharing.sh` | D — depot sharing + install plans (seed 7) |
| `bench-e-version-stream.sh` | E — many-version stream (seed 5) |
| `bench-f-io-estimate.sh` | F — local disk I/O (seed 5, 3 fixed byte flips) |
| `bench-h-route-planner.sh` | H — planner policies per client state |
| `publish-preview.sh` | Phase-4 publish preview, all routes |

The equivalent manual commands, for reference:

```sh
# A, B, C, G — pathology cases (add --butler-bin when butler is installed)
$CAVS bench steampipe-cases --out results/steampipe-cases --seed 9 --include-pairwise

# D — depot sharing: build a workspace with shared depots (see the script), then
...
$CAVS bench version-stream --out results/version-stream --size 24MiB --versions 10 --seed 5

# F — local disk I/O (dataset: seeded 256 MiB pack, 3 fixed bytes flipped)
